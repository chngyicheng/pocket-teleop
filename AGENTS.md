...
>
> **In progress — branch `feat/control-safety-fixes`:** Fixing the live-testing bugs in `docs/bugs-mission-ui.md`. **BUG 1 (stop on joystick release) is DONE** — `TeleopClient` now runs a 20 Hz continuous publisher (`PUBLISH_INTERVAL_MS`/`STOP_REPEATS` consts): while a non-zero command is held it republishes every tick so a dropped packet self-corrects; on release it sends a bounded burst of `STOP_REPEATS` (10) explicit zero-twists over ~500 ms then goes silent (keepalive ping + latency keep working). The publisher is silent with no input, so the idle-reconnect/keepalive integration behavior is unchanged, and it resets on every (re)connect so a connection blip never resumes stale motion. Both views (`MissionControl.tsx`, `MissionTablet.tsx`) now hold live axes in an `axesRef` instead of reading the cross-axis value from a stale React render closure, so releasing one joystick zeroes only its own axes and preserves the other's live value. New tests: `test/teleop_client_continuous_publish.test.ts` (3) + a cross-axis regression guard in `MissionControl.test.tsx` (1). NOTE: the real-server integration suite was NOT re-run (the live robot stack was kept up); unit/component layer is green and the change is silent-when-idle by design.
>
> **BUG 4 (real latching E-STOP, SAFETY) is DONE.** New wire protocol: client sends `{"type":"estop"}` / `{"type":"estop_reset"}`; server replies `{"type":"estop_state","engaged":bool}`. `CommandHandler` gained `EStopCommand`/`EStopResetCommand` variants; `TeleopServer` holds an `estopped_` latch — on engage it publishes one zero `cmd_vel` and **ignores all twists until reset** (ping still resets the watchdog so the connection survives while latched); the latch clears on a fresh connect. Client: `TeleopClient.engageEstop()`/`resetEstop()`, `sendTwist` is a no-op while latched, `onEstopState` callback; `useTeleopBridge` exposes `estopEngaged` + `resetEstop` (`eStop()` now engages the latch, not a single zero). Views show an "⚠ E-STOP ENGAGED" banner + the button flips to a deliberate RESET (Space engages only, never resets). Tests: server 44 (4 new estop) / webclient 286 (+11). Same baseline reds (integration needs live server + 9 adversarial + 1 whep).
>
...
>
> **Resume state (2026-06-05, latest):** the entire **`feat/control-safety-fixes`** branch (BUG 1/3/4/5 + BUG 2-joysticks) is **MERGED into `main`** via merge commit **`ae2507f`** (pushed); that branch + its worktree are deleted. The Docker stack was rebuilt from the merged code and is running (all five containers healthy). **Current work is on branch `feat/settings-drawer-redesign`** (Settings drawer UX overhaul per operator hardware feedback): drawer now slides from the **left** (matches the burger), starts **below the top bar** via a `topOffset` prop (tablet/landscape 44px, portrait 36px) so the E-STOP stays clear, the burger **toggles** open/close, a **backdrop scrim** closes it on outside tap, and it is **restyled to the Mission palette** (surface `#14171e`, amber `#f0a92a`, JetBrains Mono headings) instead of the off-theme blue. Tests: SettingsDrawer 26 / App 11. The running stack does NOT yet contain this drawer change (rebuild to test it).
>
...
>
> **BUG 3 (tablet top-bar overflow + label) is DONE.** `MissionTablet`'s E-STOP label is unified to `■ STOP` (engaged state stays `■ RESET`). The top bar stays one fixed-height (44px) line: the robot-name label is the sole shrink target (`flex:1 1 auto; min-width:0; overflow:hidden; ellipsis; nowrap`) so it truncates first, the E-STOP button is pinned `flexShrink:0` so it can never be squeezed off, and the bar has `overflow:hidden` as a safety net; readouts + connection chip keep min-content room. Single-row keeps the absolutely-positioned `top:44` engaged banner valid. Tests: 3 added to `test/MissionTablet.test.tsx`; two existing button queries moved `/E-STOP/i`→`/STOP/i`. webclient tablet suite 20 pass.
>
> **BUG 5 (fake telemetry) is DONE.** `fps`/`res` are now live: `WhepClient` polls `getStats()` every 1 s once the track is live, reads the inbound-rtp video report (`framesPerSecond`/`frameWidth`/`frameHeight`), emits via new `onStats`; `useWhepStream` exposes `stats: VideoStats | null`; tablet STREAM panel renders real fps/res with a `—` fallback before the first sample (old fake `30.1`/`1280×720` gone). Fake `UP`/`BAT`/`SIG` (tablet top bar + phone stack) now render `—` (no real source yet; battery has a backlog plan); `src`/`codec` stay static-but-accurate (`WebRTC`/`H.264`, commented); `LAT` was already real. Deviations recorded. Tests: whep_client +3, useWhepStream +2, MissionTablet +1, MissionControl +1.
>
> **Next task — all `docs/bugs-mission-ui.md` bugs are now resolved (BUG 1/3/4/5 FIXED, BUG 2 PARTIAL — E-STOP intentionally stays tappable above the drawer per operator decision; the joystick half is fixed).** Pick from the "Feature plan pool" below, or close out the branch. NOTE: the running Docker stack is still STALE (built at the BUG 1 point) — rebuild before any further hardware testing: `docker compose -p pocket-teleop --env-file /home/chngyicheng/pocket-teleop/.env down` then `... up --build -d` (the `-p` pin reuses the `auth-data` volume; credentials are NOT reset).
>
> Baseline test counts to preserve: **server 44** / **webclient 300 unit+component pass** (pre-existing reds that are NOT regressions: `integration.test.ts` needs a live server; 9 `*.adversarial.test.*` documenting other open bugs; 1 `whep_client` ICE-timer flake) / video-bridge 19 / auth 34. Feature backlog ("Feature plan pool" below) also remains available.
