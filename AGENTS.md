...
>
> **Next task — BUG 4 (real latching E-STOP, SAFETY):** today `bridge.eStop()` is a single zero twist that the very next joystick `onMove` immediately overrides — not a real stop. Add a distinct latching e-stop: a new protocol message + `CommandHandler`/`TeleopServer` state that zeroes `cmd_vel` and **ignores incoming twists until an explicit reset**, plus a UI engaged/disarm affordance. (The C++ watchdog only fires on timeout/disconnect, not on a release while still connected.) Then BUG 3 overflow + BUG 2 z-index, then BUG 5 telemetry, then BUG 3 label. Feature backlog ("Feature plan pool" below) also remains available.
