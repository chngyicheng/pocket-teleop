...
> **Current state (2026-06-07):** Gamepad controls improvements (6-task plan) merged, followed by a run of post-merge UX fixes (gamepad rAF polling, published-velocity readout, left-rail reorder, joystick sizing/scroll). Docs refreshed: **README rewritten** (banner `docs/assets/banner.svg`, screenshot placeholders in `docs/screenshots/`, controls table) and troubleshooting split into **[TROUBLESHOOTING.md](TROUBLESHOOTING.md)**. Working tree clean; **~9 commits unpushed** on `main` (ahead of `origin/main`). **Considering a `v1.0.0` tag** — gated on operator verifying held-stick continuous drive + axis directions on the real robot (see hardware-verify items in [deviations.md](memory/agent-guides/deviations.md)). Per-feature rationale lives in deviations + the Milestones rows — this block stays orientation-only.
>
