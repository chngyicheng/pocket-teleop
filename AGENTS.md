...
> **For the next agent:** Video source picker UI wired, all tests pass (33 auth-server / 99 webclient / 19 video-bridge), but **"Apply" button on Video settings page returns HTTP 404 at runtime** — PATCH to `/mediamtx-api/config/paths/patch/teleop` never reaches MediaMTX. Three fix attempts committed (see Known deviations below and commits `9104493`, `916490d`, `01373bc`) — none resolved runtime 404. Next agent must investigate live request path (auth-server → MediaMTX port 9997) before writing more code. Tests don't cover runtime path (only assert response is not 302), so passing tests don't confirm fix works.
...
| **OPEN BUG: `/mediamtx-api` PATCH returns 404 at runtime** | `auth-server/src/app.ts`, `web-client/src/video_source.ts` | Three fixes attempted (`9104493` wrong port, `916490d` pathRewrite, `01373bc` manual req.url mutation) — all pass tests but none resolve runtime 404. Tests only check response is not 302; don't verify request reaches MediaMTX. Next step: add docker logs / curl tracing to confirm whether request reaches port 9997 and what MediaMTX returns. |
