...
> **To next agent (2026-06-05, current):** Everything below is **on `main`** (latest `00f14b4` = the collapsible-rails merge, pushed). There is **no active feature branch** — start fresh from `main` (branch first before committing; see Execution mode). Working tree carries only untracked `.claude/` and `design_handoff_pocket_teleop/` (design reference, not built). The full per-task detail lives in the **Milestones done** table below and in `docs/bugs-mission-ui.md`; this block is just the orientation.
>
> **⚠ NEXT TASK — three live bugs diagnosed, plan written, NOT yet built.** Operator hit these after the collapsible-rails merge; root causes are confirmed (see plan). **Build them next:** `docs/superpowers/plans/2026-06-05-post-merge-bugfixes.md` (worktree + Haiku subagents + trophy TDD, three independent tasks A/B/C).
> 1. **~15 s white screen after login (offline LAN only).** `web-client/index.html` keeps a render-blocking external Google Fonts `<link>` (Inter + JetBrains Mono) + 2 preconnects. On a robot LAN with no internet the browser blocks first paint until that request times out → the SPA stays blank ~15 s. Login page feels fast (static + `display=swap`). **Fix:** vendor the woff2 into `web-client/public/fonts/` + `@font-face` in `src/index.css`, delete the external links. (The local dev box has internet so it loads instantly — only no-internet deployments hang; verify with the network blocked.)
> 2. **Login / Change-password pages are off-theme.** `auth-server/views/login.html` + `change-password.html` are white-bg / blue-accent / `Press Start 2P`, never migrated to the Mission palette the rest of the app uses; they ALSO carry the same external Google Fonts link. **Fix:** restyle to Mission palette (`#0c0e12`/`#14171e`/amber `#f0a92a`/JetBrains Mono) + serve fonts from an **unauthenticated** auth static route (mounted BEFORE the auth-redirect middleware in `app.ts`).
> 3. **admin/admin no longer forces a credential change.** `credentials.ts:initCredentials` only writes `mustChangePassword:true` when the file is absent; the persistent `auth-data` volume (kept alive by the `-p pocket-teleop` pin) still holds `{username:'admin', mustChangePassword:false}`, so the default login walks straight into the app. Confirmed by reading `/data/credentials.json` in the running container. **Fix:** add a startup check — if the stored hash still verifies the env `TELEOP_ADMIN_PASSWORD` (operator still on default), force `mustChangePassword:true` + atomic-save; document `docker compose down -v` as the hard credential reset.
>
...
>
> **Test baseline to preserve:** **server 44** / **webclient 323 unit+component pass** / **video-bridge 19** / **auth 34**. Pre-existing reds that are NOT regressions (leave red): `integration.test.ts` (needs a live server), 9 `*.adversarial.test.*` (document other open bugs by design), 1 `whep_client` "ICE gathering timer cleanup" flake. Tests run in Docker only (see [repository-structure.md](memory/agent-guides/repository-structure.md)); iterate with a targeted file list.
>
> **Next task — operator to pick.** No bug backlog remains. Options: (a) a feature from the **Feature plan pool** below (HTTPS/TLS is the standout safety gap for a networked robot; battery telemetry would retire the `BAT —` placeholder from BUG 5); (b) housekeeping — a stale **`feat/mission-ui`** local branch and an unregistered **`.worktrees/feat-mission-ui`** directory are left over from earlier work and can be deleted.
...
| **⚠ Post-merge bugfixes plan (2026-06-05, NEXT — not built)** | `docs/superpowers/plans/2026-06-05-post-merge-bugfixes.md` |
