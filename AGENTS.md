...
> **To next agent (2026-06-05, current):** The **video lazy-load + NO SIGNAL + faster first paint** feature is **DONE — merged to `main` and pushed** (was branch `feat/video-lazy-load`, now deleted; worktree removed). Working tree on `main` carries only untracked `.claude/` and `design_handoff_pocket_teleop/`. Per-task detail in the **Milestones done** table below; this block is orientation. The white-screen investigation resolved to a **serving-layer fix (nginx gzip was off)** — see 戊 below; not the video, not JS execution.
>
...
> - **丁 — instant boot splash + white-screen diagnosis.** The phone perf beacon revealed the white screen is **NOT the video** — it's the **190 KB module bundle download+execute before first paint**: a real Android load reported `responseEndMs 633` (HTML in) → `domContentLoadedMs 7891` / `readyMs 7919` (UI painted) = **~7.3 s blank**, which the video deferral cannot touch (it's upstream of mount). Two responses: (1) **instant splash** — hand-written `#boot-splash` markup + critical inline `<style>` (Mission dark `#0c0e12`, amber spinner) in `index.html`, painting on HTML parse (~0.6 s) so no white flash; React `createRoot` overwrites `#app` on first commit. (2) **diagnosis fields** added to the beacon (`bundleMs` = bundle responseEnd−startTime, `bundleTransferMs`, `bundleKB`) to split the cost into WiFi-download vs JS-execute.
> - **戊 — ROOT CAUSE FOUND + FIXED: nginx gzip was off.** The diagnostic beacon on the next real load reported `bundleTransferMs 1717` (1.7 s on the wire), `domContentLoaded − bundle-end ≈ 20 ms` (JS execute is **trivial**, NOT the bottleneck), and the smoking gun `bundleKB 186` — the bundle **gzips to ~58 KB at build but shipped at 186 KB uncompressed** because `nginx:alpine` defaults gzip **off** and the image carried no custom config. Fix: new `web-client/nginx.conf` (`gzip on` for JS/CSS/JSON/SVG + `immutable` cache for hashed `/assets/` + `no-cache` on `index.html`), `COPY`-ed to `/etc/nginx/conf.d/default.conf` in `Dockerfile.webclient`. Verified on the rebuilt image: bundle now serves `Content-Encoding: gzip` (186→~58 KB, ~3×) + immutable cache (repeat loads skip download). Also: the same load showed the splash working — `fcpMs 500` (was `null`), `readyMs 7919→2303`. **No Preact / React.lazy needed** — it was a serving-layer config, not an app-weight problem.
> - **+34 tests** (甲 useWhepStream +3; 乙 MissionTablet +6 / MissionControl +11; 丙 perf_beacon webclient +6 / auth +3; 丁 boot_splash +2; 戊 nginx_gzip +3). All green (363 webclient pass / 49 auth pass). Pre-existing reds unchanged (webclient: 9 adversarial + 1 whep ICE-timer flake; auth: 2 adversarial); none in touched files.
>
...
>
> **Next task — operator to pick.** No bug backlog remains (video lazy-load + NO SIGNAL + gzip first-paint fix is merged + pushed). Options: (a) a feature from the **Feature plan pool** below (HTTPS/TLS is the standout safety gap for a networked robot; battery telemetry would retire the `BAT —` placeholder from BUG 5); (b) the remaining first-load latency is the HTML round-trip on a variable Wi-Fi link — a **service worker** precaching the app shell would make repeat loads render before the network responds (separate feature, has staleness/deploy tradeoffs); (c) housekeeping — a stale **`feat/mission-ui`** local branch and an unregistered **`.worktrees/feat-mission-ui`** directory are left over from earlier work and can be deleted.
...
| Video lazy-load + NO SIGNAL — **甲** `useWhepStream` defers `WhepClient.start()` to after first paint (`requestIdleCallback`→`setTimeout` fallback; `cancelled` flag + handle-cancel cleanup = no leak on early unmount), production path `await import('../whep_client.js')` → Vite splits `whep_client-*.js` chunk (2.95 KB / 1.14 KB gzip), injected-factory path stays synchronous (existing tests intact), top `import type`. Initial JS 191031→189988 B (~1 KB; real win is deferred start, honest in deviations). **乙** new `VideoSignalOverlay` in `shared.tsx` (`stream.state`→`CONNECTING…`/`RECONNECTING…`/`NO SIGNAL`, live→none), placed in all 3 video containers, `pointerEvents:'none'` zIndex 2 < joystick z 5, joystick show/hide untouched (safety). 2 Haiku parallel (wenyan-ultra, disjoint files) + trophy TDD; controller hoisted shared.tsx import + verified code-split via real build. **丙** UI-ready perf beacon (operator request): `perf_beacon.ts` POSTs Navigation/Paint/resource Timing after first paint (double-rAF in `main.tsx`) to new unauthenticated `POST /perf` in auth `app.ts`, logged `[perf] <iso> <json>` (read via `docker logs pocket-teleop-auth-server-1 | grep perf`). **丁** instant boot splash: hand-written `#boot-splash` + critical inline `<style>` (Mission dark `#0c0e12`, amber spinner) in `index.html`, paints on HTML parse so no white flash; React `createRoot` overwrites `#app` on first commit. **戊 — root cause: nginx gzip was off.** Beacon proved white screen = 190 KB bundle shipped **uncompressed** (gzips to ~58 KB; `nginx:alpine` defaults gzip off, no custom conf), JS execute ~20 ms (not it). New `web-client/nginx.conf` (`gzip on` + `/assets/` immutable cache + `index.html` no-cache) `COPY`-ed in `Dockerfile.webclient`; verified `Content-Encoding: gzip` 186→~57 KB; real loads improved `readyMs` 7919→~1937 ms, bundle now cache-served (0 ms). +34 tests (useWhepStream +3, MissionTablet +6, MissionControl +11, perf_beacon webclient +6 / auth +3, boot_splash +2, nginx_gzip +3). | 49 auth pass (+3; 2 adversarial red) / 363 webclient pass (+34; same 10 baseline reds) / 19 video-bridge / 44 C++ | `feat/video-lazy-load` (merged to `main`) |
