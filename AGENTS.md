...
> **Current state (2026-06-10):** **SLAM minimap shipped** (6-task plan, chain-merged): the MiniMap now renders the real SLAM occupancy grid (transient_local `/map`, crop window, trinary RLE), the tf2 `map→base_link` pose (odom fallback) and a decimated lidar overlay, base_link-fixed (robot centered, map rotates — RViz style). All topics/frames deployer-configurable via env (`MAP_TOPIC`, `SCAN_TOPIC`, `ODOM_TOPIC`, `MAP_FRAME`, `ODOM_FRAME`, `BASE_FRAME`, `MAP_WINDOW_M`). Server side **live-verified against the real robot** (map 93×115 RLE 8×, pose frame=map, scan 120 pts over WS via throwaway node on :19099). **Screen-direction conventions still need operator eyes** (drive forward → map flows down, etc. — hardware-verify rows in [deviations.md](memory/agent-guides/deviations.md)). Earlier state: gamepad 6-task plan + post-merge UX fixes + README/TROUBLESHOOTING refresh. Commits unpushed on `main`. `v1.0.0` tag still pending operator hardware checks. Per-feature rationale lives in deviations + the Milestones rows — this block stays orientation-only.
>
...
>
> **Test baseline:** webclient **548** pass / **11** skipped / auth **51** pass / video-bridge **19** / C++ **69**. Docker only (see [repository-structure.md](memory/agent-guides/repository-structure.md); iterate with a targeted file list) — rebuild with `--build` after editing tests, else `docker compose run` reuses a stale baked image. Adversarial backlog (9 webclient + 2 auth) cleared — see latest **Milestones** row. The two former webclient reds are now fixed: `whep_client` ICE-timer test rewritten (functional mock listener registry + `vi.getTimerCount`), and `integration.test.ts` now `describe.skipIf(!serverReachable)` (probes once, **skips** without a live server instead of hard-failing; run the full `--profile test` stack to exercise it). Remaining pre-existing red — leave red, not a regression: auth `mediamtx_integration.test.ts` (3 tests; needs the `mediamtx-test` companion container — green only via full `--profile test` compose).
>
...
| **SLAM minimap (2026-06-10, ✅ done — real map + tf2 pose + lidar overlay, base_link-fixed canvas, env-config topics; webclient 548 / C++ 69)** | `docs/superpowers/plans/2026-06-10-slam-minimap.md` |
