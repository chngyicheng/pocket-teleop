...
> - **Collapsible side rails + video auto-fit** (latest, branch `feat/collapsible-rails-video-fit`) — new `CollapsibleRail` presentational component (toggle tab + chevron at the inner edge, `aria-expanded`, `data-testid rail-tab-{side}`). **Tablet** left (STREAM) / right (MAP) `<aside>`s wrap it; the grid `gridTemplateColumns` is now state-driven (`220/240px ↔ 22px`, `transition: grid-template-columns 0.2s`) so tapping a tab slides the rail shut and the `1fr` center widens. **Phone-landscape was restructured** from floating overlays into the same real collapsible left/right rails (`180px ↔ 22px`); joysticks moved inside the center `<main>` so they no longer sit over the rails. **Phone-portrait is unchanged** (floating overlays — guarded by a regression test). Both video elements: `objectFit: 'cover' → 'contain'` (fits whichever of width/height is smaller, letterboxes, no crop; grows as rails collapse).
>
...
| Collapsible side rails + video auto-fit — new `CollapsibleRail` (presentational: inner-edge toggle tab + side-aware chevron, `aria-expanded`, `data-testid rail-tab-{side}`). `MissionTablet` left (STREAM) / right (MAP) rails wrap it; `gridTemplateColumns` state-driven `220/240↔22px` + `transition` so a tab tap slides the rail shut and the `1fr` center widens. `MissionControl` **phone-landscape restructured** from floating overlays into real collapsible rails (`180↔22px`), joysticks moved into center `<main>`; **portrait unchanged** (regression-guarded). Both videos `objectFit cover→contain` (fit smaller of W/H, letterbox, grows on collapse). 3 Haiku (T1 component / T2 tablet / T3 landscape) sequential in one worktree; controller DRY-restored tablet right rail to `SidePanel`. +17 tests (CollapsibleRail 8 unit, MissionTablet +5, MissionControl +4 integration). | 34 auth / 323 webclient pass (+17) / 19 video-bridge / 44 C++ | `feat/collapsible-rails-video-fit` |
