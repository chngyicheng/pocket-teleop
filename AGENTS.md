...
> **致下一代理：** Mission Control UI React 移植役 Wave 1–3 皆竣（branch `feat/mission-ui-react`，worktree `.worktrees/feat-mission-ui-react`，自 `feat/mission-ui` HEAD `5775462` 分出）。原計劃 `docs/superpowers/plans/2026-05-28-mission-ui-react-migration.md`。**Phase 0**（`dea3f83`）vanilla TS 殼刪、React 18 + Vite 5 + jsdom 配。**Wave 1**（`95d6d42`）三 Haiku 並寫 `shared.tsx`/`useTeleopBridge`/`useWhepStream` factory hooks/`SettingsDrawer.tsx`，控制器於 `test/setup.ts` 加 RTL cleanup + MediaStream/PointerEvent/setPointerCapture/getBoundingClientRect 諸 polyfill。**Wave 2**（`38d2095`）二 Haiku 並寫 `views/MissionControl.tsx` phone layout 與 `views/MissionTablet.tsx` tablet 三欄 grid。**Wave 3** 單 Haiku 寫 `src/App.tsx`（matchMedia tablet ≥ 900px 切、條件渲 `<MissionTablet>` vs `<MissionControl>`、SettingsDrawer 整合）+ `src/main.tsx` + `src/index.css` + `test/App.test.tsx`（crown jewel 9 集成測）+ 改 `index.html`（Google Fonts CDN + index.css link）。控制器修諸事：(a) 加 matchMedia 與 dispatchEvent-vs-fireEvent 之 Space 測修、(b) MissionControl/MissionTablet hamburger 自 `<div>` 改 `<button aria-label="Open menu">`、(c) connection chip 測接 "Live" 或 "Connected" 兩文本形（landscape vs portrait）、(d) tablet layout 測自 `queryByText` 改 `queryAllByText` 以容多匹配、(e) App test 自 setup.ts 之 matchMedia 反裝至 test scope 內（用戶意 setup.ts 簡）。**Trophy TDD：** Wave 1 之 80 + Wave 2 之 15 + Wave 3 之 9 = 104 新測。**測：** 268/269 webclient（1 pre-existing whep_client ICE-timer flake 於 d5fb6cd 主同失，環境計時敏感）/ 34 auth / 19 video-bridge / 40 C++ 皆通。**用戶煙測（瀏覽器）：** 通過，總體可用，**然發現八處 design 對等出入**（四功能缺、三視覺缺、一 phone-portrait 佈局回歸）。詳見新計劃 `docs/superpowers/plans/2026-05-30-mission-ui-design-parity-fixes.md`。
>
