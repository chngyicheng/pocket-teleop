...
> **致下一代理：** Mission Control UI React 移植役進行中（branch `feat/mission-ui-react`，worktree `.worktrees/feat-mission-ui-react`，自 `feat/mission-ui` HEAD `5775462` 分出）。計劃 `docs/superpowers/plans/2026-05-28-mission-ui-react-migration.md`。**Phase 0 竣**（`dea3f83`）：vanilla TS mission 殼諸文件刪、React 18 + Vite 5 + vitest jsdom 配。**Wave 1 竣**（`95d6d42`）：三 Haiku 並派 `shared.tsx`（Joystick + 五 HUD 組件 + CONNECTION_LABELS）、`useTeleopBridge`/`useWhepStream` factory-form hooks、`SettingsDrawer.tsx` slide-in 三 section；控制器於 `test/setup.ts` 加 RTL cleanup + MediaStream/PointerEvent/setPointerCapture/getBoundingClientRect 諸 polyfill；改 ctor 為 factory；修諸 CSS 屬性選擇器 camelCase→kebab；`shared.tsx` MiniMap `background` 改 `backgroundImage` 長手 + `hexToRgba()` helper；去諸 silent-pass `if` guard。**Wave 2 竣：** 二 Haiku 並派各於 disjoint 文件域：①Agent D — `src/views/MissionControl.tsx`（phone layout，phone-landscape vs phone-portrait 切；自 `design_handoff_pocket_teleop/directions/mission.jsx` 之 `MissionControl` 函數 1:1 移；含真 `<video>` 元素自 `stream.stream` 取、Joystick onMove 軸映射 DRIVE `sendTwist(-y, 0, -x)` + STRAFE `sendTwist(_, x, _)`、E-STOP 鈕 z-index 10 + Space 鍵 keydown 雙觸 `bridge.eStop()`、hamburger → `onMenu`、telemetry stack LAT 自 `bridge.latencyMs`、BAT/SIG 暫固定 UI-only）+ `test/MissionControl.test.tsx`（7 測）；②Agent E — `src/views/MissionTablet.tsx`（tablet CSS grid 220px/1fr/240px + 52px top bar 跨全；六 SidePanel：STREAM/VELOCITY/ODOMETRY/MAP/HEADING/LIGHTS；中央 reticle 80×80 SVG + V/ω readouts；二 Joystick overlay 底角 size 280 z-index 5；LIGHTS 三 MissionPillToggle UI-only + console.log；Space + E-STOP 同 MissionControl）+ `test/MissionTablet.test.tsx`（8 測）。**控制器修：** `MissionControl.test.tsx` Space 測自 `fireEvent.keyDown(window, event)` 改 `window.dispatchEvent(event)`（前者捨原 event instance 致 `preventDefault` spy 永未呼）。**Trophy TDD：** Wave 1 之 80 + Wave 2 之 15 = 95 新測。**測：** 259/260 webclient（1 pre-existing whep_client ICE-timer flake 於 d5fb6cd 主同失，環境計時敏感）/ 34 auth / 19 video-bridge / 40 C++ 皆通。**未竣:** Wave 3（`App.tsx` + `main.tsx` + `index.html` rewrite + crown jewel 集成測 ≥ 8）。
>
