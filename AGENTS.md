...
>
> **Next task — BUG 2/3 (E-STOP + joystick stacking vs Settings drawer):** the Settings drawer (right slide-in, `z-index ≤ 9`) is painted UNDER the E-STOP button (`zIndex:10`) and under the touch joysticks, and the joysticks stay interactive while the drawer is open. Fix: raise the drawer's stacking context above both, and while the drawer is open make the joysticks non-interactive (`pointerEvents:none`) while keeping their hints visible one layer below the drawer. Also BUG 3 overflow (tablet top-bar E-STOP bleeds off-screen) + BUG 3 label unification ("STOP" vs "E-STOP"). Then BUG 5 telemetry. Feature backlog ("Feature plan pool" below) also remains available.
