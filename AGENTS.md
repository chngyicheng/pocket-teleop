...
>
> **Test baseline:** webclient **787** pass / **11** skipped / auth **96** / video-bridge **20** / C++ **88**. Docker only; `--build` required after edits. Known non-regression reds: auth `mediamtx_integration.test.ts` (3, needs `--profile integration` + live MediaMTX); `integration.test.ts` self-skips without a live server.
>
