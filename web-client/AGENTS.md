...
| `geofence.ts` | Pure keep-out polygons (map frame): `pointInPolygon` (ray-cast), `distanceToBoundary`, `closestBoundaryPoint`, `speedScale` (buffer ramp), `twistScale(pose, lin, fences)` → per-axis `{lin, az}` with **escape mode** — inside a fence rotation stays free (az 1) and linear motion runs at `ESCAPE_SPEED_SCALE` (0.3) only when the body→world-rotated command points toward the nearest boundary (a nosed-in robot can always back/turn out; deeper-in stays 0). Publisher tick applies it output-side; slew ramp untouched. nav2 guards live in the bridge: goals inside a fence are rejected at Send, and a mid-run breach auto-fires `nav_cancel` + error toast (nav2's own cmd_vel is otherwise unconstrained — deviations) |
